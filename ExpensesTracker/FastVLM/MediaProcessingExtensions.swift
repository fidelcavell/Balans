...
enum MediaProcessingExtensions {
    ...
    /// Fits the image inside `size` maintaining aspect ratio, then centers it on a black canvas of dimension `size`.
    /// This avoids cropping out receipt headers or bottom totals while strictly guaranteeing `extent.size == size`.
    public static func padToSquare(_ image: CIImage, size: CGSize) -> CIImage {
        var normalized = image
        if normalized.extent.origin != .zero {
            normalized = normalized.transformed(
                by: CGAffineTransform(translationX: -normalized.extent.origin.x, y: -normalized.extent.origin.y)
            )
        }

        let scale = min(size.width / normalized.extent.width, size.height / normalized.extent.height)
        let newWidth = max(1, (normalized.extent.width * scale).rounded())
        let newHeight = max(1, (normalized.extent.height * scale).rounded())
        let scaledImage = resampleBicubic(normalized, to: CGSize(width: newWidth, height: newHeight))

        let dx = ((size.width - newWidth) / 2.0).rounded()
        let dy = ((size.height - newHeight) / 2.0).rounded()
        let centered = scaledImage.transformed(by: CGAffineTransform(translationX: dx, y: dy))

        let canvas = CGRect(origin: .zero, size: size)
        let blackBg = CIImage(color: .black).cropped(to: canvas)
        return centered.composited(over: blackBg).cropped(to: canvas)
    }
    ...
    public static func fitIn(_ size: CGSize, longestEdge: Int) -> CGSize {
        ...
        guard maxDim > floatLongestEdge, maxDim > 0 else {
            return size
        }
        let scale = floatLongestEdge / maxDim
