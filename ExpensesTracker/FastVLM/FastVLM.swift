...
private enum Language {
    ...
    nonisolated fileprivate class Attention: Module {
    ...
    nonisolated fileprivate class MLP: Module, UnaryLayer {
    ...
    nonisolated fileprivate class FastVLMDecoderLayer: Module {
    ...
    nonisolated fileprivate class Qwen2Model: Module {
    ...
    nonisolated fileprivate class LanguageModel: Module, KVCacheDimensionProvider {
...
private enum Vision {
    ...
    nonisolated fileprivate class VisionModelCoreML {
        ...
        public func encode(_ image: MLXArray) -> MLXArray {
            ...
            precondition(image.dim(2) == 1024, "Image height must be 1024, got \(image.dim(2))")
            precondition(image.dim(3) == 1024, "Image width must be 1024, got \(image.dim(3))")
    ...
    nonisolated fileprivate class VisionModel: Module {
...
extension UserInput.Prompt {
    nonisolated fileprivate func asMessages() -> [[String: String]] {
        switch self {
        case .text(let text):
...
nonisolated public final class FastVLMProcessor: UserInputProcessor {
    ...
    public init(_ config: FastVLMPreProcessorConfiguration, tokenizer: any MLXLMCommon.Tokenizer) {
    ...
    public func preprocess(image: CIImage, processing: UserInput.Processing?) throws -> (
        ...
        let targetSize = imageProcessingConfig.cropSize.size

        // Pad to square so receipts are never cropped,
        // ensuring headers and bottom totals remain fully visible while strictly maintaining targetSize (1024x1024).
        image = MediaProcessingExtensions.padToSquare(image, size: targetSize)
...
nonisolated private class FastVLMMultiModalProjector: Module, UnaryLayer {
...
nonisolated public class FastVLM: Module, VLMModel, KVCacheDimensionProvider {
    ...
    static public var modelConfiguration: ModelConfiguration {
        ...
        let possibleUrls = [
            ...
            bundle.url(forResource: "config", withExtension: "json", subdirectory: "model"),
            bundle.url(forResource: "config", withExtension: "json", subdirectory: "ExpensesTracker/FastVLM/model"),
            mainBundle.url(forResource: "config", withExtension: "json"),
            mainBundle.url(forResource: "config", withExtension: "json", subdirectory: "FastVLM/model"),
            mainBundle.url(forResource: "config", withExtension: "json", subdirectory: "model"),
            mainBundle.url(forResource: "config", withExtension: "json", subdirectory: "ExpensesTracker/FastVLM/model")
        ]

        for candidate in possibleUrls {
            if let candidate {
                let dir = candidate.resolvingSymlinksInPath().deletingLastPathComponent()
                return ModelConfiguration(directory: dir)
            }
        }

        if let resourceURL = bundle.resourceURL ?? mainBundle.resourceURL {
            let candidates = [
                resourceURL.appendingPathComponent("FastVLM/model"),
                resourceURL.appendingPathComponent("model"),
                resourceURL.appendingPathComponent("ExpensesTracker/FastVLM/model"),
                resourceURL
            ]
            for candidate in candidates {
                if FileManager.default.fileExists(atPath: candidate.appendingPathComponent("config.json").path) {
                    return ModelConfiguration(directory: candidate)
                }
            }
        }

        return ModelConfiguration(directory: bundle.resourceURL ?? bundle.bundleURL)
    }
...
nonisolated public struct FastVLMConfiguration: Codable, Sendable {
...
nonisolated public struct FastVLMPreProcessorConfiguration: Codable, Sendable {
...
nonisolated public struct FastVLMProcessorConfiguration: Codable, Sendable {

    nonisolated public enum Strategy: String, Codable, Sendable, Equatable {
        case `default`
    }
